	...
	static {
		for (int i = 0; i < 20; i++) {
			try {
				...
				Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/mytwitter?useUnicode=true&characterEncoding=UTF-8&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC&useSSL=false&allowMultiQueries=true", "root", "019012");
				conPool.add(con);
			} catch (Exception e) {
				e.printStackTrace();
			}
